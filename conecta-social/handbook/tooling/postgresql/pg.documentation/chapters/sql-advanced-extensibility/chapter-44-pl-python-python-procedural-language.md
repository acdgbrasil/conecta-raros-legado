...
what users can do in it and is therefore named plpython3u. A trusted variant plpython might
...
administrator. Only superusers can create functions in untrusted languages such as plpython3u.
Note
...
and the Python worlds. This informs the data mapping rules described below.
44.2.1. Data Type Mapping
When a PL/Python function is called, its arguments are converted from their PostgreSQL data type
...
• When the PostgreSQL return type is bytea, the return value will be converted to Python bytes
using the respective Python built-ins, with the result being converted to bytea.
...
```
Strings are automatically converted to the PostgreSQL server encoding when they are passed to
PostgreSQL.
• For nonscalar data types, see below.
...
```
44.2.2. Null, None
If an SQL null value is passed to a function, the argument value will appear as None in Python. For
example, the function definition of pymax shown in Section 44.1 will return the wrong answer for
null inputs. We could add STRICT to the function definition to make PostgreSQL do something more
```
...
PL/Python function, return a Python list:
```
...
value, it has the value None in the mapping. Here is an example:
```
...
$$ LANGUAGE plpython3u;
```
Any extra dictionary key/value pairs are ignored. Missing keys are treated as errors. To return an
...
A PL/Python function can also return sets of scalar or composite types. There are several ways to
...
```
The global dictionary GD is public data, that is available to all Python functions within a session; use
```
with care.
...
The PL/Python language module automatically imports a Python module called plpy. The functions
and constants in this module are available to you in the Python code as plpy.foo.
...
If limit is specified and is greater than zero, then plpy.execute retrieves at most limit
rows, much as if the query included a LIMIT clause. Omitting limit or specifying it as zero
...
empty result object. Cursor objects also provide an iterator interface2, yielding one row at a time
...
$$ LANGUAGE plpython3u;
```
2 https://docs.python.org/library/stdtypes.html#iterator-types
1487
PL/Python — Python
Procedural Language
...
the Python Database API specification3. They don't have anything in common except for
the name.
44.6.2. Trapping Errors
Functions accessing the database might encounter errors, which will cause them to abort and raise
an exception. Both plpy.execute and plpy.prepare can raise an instance of a subclass of
plpy.SPIError, which by default will terminate the function. This error can be handled just like
any other Python exception, by using the try/except construct. For example:
```
...
Recovering from errors caused by database access as described in Section 44.6.2 can lead to an unde-
...
```
ager interface4. Using explicit subtransactions we can rewrite our function as:
```
...
plpy.execute(plan, [result])
```
4 https://docs.python.org/library/stdtypes.html#context-manager-types
1489
PL/Python — Python
Procedural Language
```
$$ LANGUAGE plpython3u;
```
Note that the use of try/except is still required. Otherwise the exception would propagate to the top
...
```
In a procedure called from the top level or an anonymous code block (DO command) called from the top
```
```
level it is possible to control transactions. To commit the current transaction, call plpy.commit().
```
```
...
```
Another set of utility functions are plpy.quote_literal(string), plpy.quote_nul-
```
```
lable(string), and plpy.quote_ident(string). They are equivalent to the built-in quot-
```
ing functions described in Section 9.4. They are useful when constructing ad-hoc queries. A PL/Python
equivalent of dynamic SQL from Example 41.1 would be:
```
...
PL/Python supports only Python 3. Past versions of PostgreSQL supported Python 2, using the
plpythonu and plpython2u language names.
