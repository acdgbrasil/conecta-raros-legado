...
```
INSERT, UPDATE, DELETE, or MERGE; and the WITH clause itself is attached to a primary statement
```
that can also be a SELECT, INSERT, UPDATE, DELETE, or MERGE.
138
Queries
...
```
You can use data-modifying statements (INSERT, UPDATE, DELETE, or MERGE) in WITH. This
```
allows you to perform several different operations in the same query. An example is:
```
WITH moved_rows AS (
```
